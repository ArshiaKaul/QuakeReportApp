...
public final class QueryUtils {
    ...
    private static List<Earthquake> extractFeaturesFromJSON(String earthquakeJSON) {
        ...
        try {
            ...
            JSONArray jsonFeatures = jsonRoot.optJSONArray("features");
            for( int i=0; i<jsonFeatures.length(); i++){
                JSONObject jsonEarthquake = jsonFeatures.optJSONObject(i);
                JSONObject jsonProperties = jsonEarthquake.optJSONObject("properties");
                Double jsonMagnitude = jsonProperties.optDouble("mag");
                String jsonPlace = jsonProperties.optString("place");
                Long jsonTime = jsonProperties.optLong("time");
                String jsonURL = jsonProperties.optString("url");
                Earthquake earthquakeObj = new Earthquake(jsonMagnitude, jsonPlace, jsonTime, jsonURL);
